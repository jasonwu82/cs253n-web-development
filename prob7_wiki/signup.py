...
def isValidUserToken(username,token):
    res = True
    if not username:
        res = False
    ...
    # check if user is in database
        ...
        res = False
    elif not db_users.verify_token(username, token): 
        res = False
    if not res:
       logging.info("not valid username and token in cookie") 
       return False
    return True
class Welcome_handler(webapp2.RequestHandler):        
    ...
    def get(self):
        ...
        if not isValidUserToken(username,token):
